...
from roitr_cpp_wrappers.pointops.functions.pointops import knnquery
import glob, os
from sklearn.neighbors import NearestNeighbors
import cv2
        

def remove_outliers(pc, rad):
    '''
    This method removes the outliers from a point cloud
    :param pc: input point cloud of shape [n,3]
    :param rad: radius of the sphere that defines the inliers

    :return: pc with removed outliers
    ''' 
    center = np.mean(pc, axis=0)
    distances = np.linalg.norm(pc-center, axis=1)
    std_dist = np.std(distances)
    mean_dist = np.mean(distances)
    inside_sphere_mask = distances <= mean_dist + 2*std_dist
    pc_filtered = pc[inside_sphere_mask]

    center_new = np.mean(pc_filtered, axis=0)
    distances = np.linalg.norm(pc-center_new, axis=1)
    inside_sphere_mask = distances <= rad
    pc_clean = pc[inside_sphere_mask]
    return pc_clean, center_new, inside_sphere_mask
      

def calc_pc_radius(pc):
    '''
    This method calculates the radius of a point cloud
    :param pc: input point cloud of shape [n,3]
...
def distance_to_nearest_neighbor(points, k=1, sample_size=99999):
    '''
    This method calculates the average distance to the nearest k neighbors in a point cloud.
    :param points: point cloud of shape [n, 3]
    :param sample_size: Only calculates distances for a certain sample size

    :return: Per vertex distance to the n nearest neighbors
    '''
    if sample_size is None or sample_size > len(points):
        sample_size = len(points)
    sample_indices = np.random.choice(len(points), sample_size, replace=False)
    sampled_points = points[sample_indices]

    nn_model = NearestNeighbors(n_neighbors=k+1)  # +1 to include self
    nn_model.fit(points)
    distances, _ = nn_model.kneighbors(sampled_points, n_neighbors=k+1, return_distance=True)
...
def rle_decode(rle, shape):
    '''
    This method decodes a rle binary mask
    :param rle: (shape[0]*shape[1])
    :param shape: the shape of the original binary mask

    :return: Binary mask in shape: (shape[0], shape[1])
    '''
    mask = np.zeros(shape[0] * shape[1], dtype=np.uint8)
    current_position = 0
    value = 0
    for i in range(0, len(rle)):
        end = current_position + rle[i]
        mask[current_position:end] = value
        current_position += rle[i]
        if value == 0:
            value = 1
        else:
            value = 0
...
def pps_sampling(points, num_samples):
    def compute_density(point_cloud, k=10):
        distances = torch.cdist(point_cloud, point_cloud)
        knn_distances, _ = torch.topk(distances, k=k+1, largest=False)
        avg_distances = torch.mean(knn_distances[:, 1:], dim=1)  
        densities = 1. / (avg_distances + 1e-10) 
        return densities

    importances = compute_density(points)
    probabilities = importances / torch.sum(importances)

    sampled_indices = torch.multinomial(probabilities, num_samples, replacement=False)
    return sampled_indices


def depth_to_world(depth, cam_k, depth_scale, bbox, mask=None, augment=False):
    '''
    This method transforms a part of the input depth image into a point cloud.
    :param depth: depth image of shape [w, h]
    :param cam_k: intrinsic camera parameters of shape [3,3]
    :param depth_scale: depth scale of the depth image
    :param bbox: the region of the depth image that is to be transformed into a point cloud
    :param mask (optional): the region within the bbox that is to be transformed into a point cloud
    :param augment: Whether to include a portion of the pixels outside the mask as distractor vertices
    :return: point cloud array
    '''
    
    cam_K_inv = np.linalg.inv(cam_k)
    x_range = np.arange(bbox[0], bbox[0] + bbox[2] + 1)
    y_range = np.arange(bbox[1], bbox[1] + bbox[3] + 1)
    X, Y = np.meshgrid(x_range, y_range)
    X, Y = X.flatten(), Y.flatten()

    depths = depth[X, Y] * depth_scale
    valid_depths = depths != 0
    X, Y, depths = X[valid_depths], Y[valid_depths], depths[valid_depths]
    pixels = np.vstack((X, Y, np.ones_like(X))) * depths
    
    points = cam_K_inv @ pixels
    if mask is not None:
        mask_flat = mask[X, Y]
        pc = points[:, mask_flat != 0].T
        pc_distractor = points[:, mask_flat == 0].T
    else:
        pc = points.T
        pc_distractor = np.array([]).reshape(0, 3)

    if augment and len(pc_distractor) > 3:
        num_distractors = min(int(np.random.uniform(0, 0.2) * len(pc)), len(pc_distractor))
        distractor_indices = np.random.choice(len(pc_distractor), num_distractors, replace=False)
        pc_distractor_sample = pc_distractor[distractor_indices]
        pc = np.vstack((pc, pc_distractor_sample))

    return pc.astype(np.float32)


def to_ply_file(p1, p2, name1="src_pcd.ply", name2="tgt_pcd.ply"):
    '''
    Saves the two given point clouds in ply format
    :param p1 / p2: poincloud 1 / 2
    :param name1 / name2: output filename of ply-file 1 / 2
    :return: None
    '''
    if type(p1) == torch.Tensor:
        p1 = p1.cpu().numpy()
    if type(p2) == torch.Tensor:
        p2 = p2.cpu().numpy()
    ...
    o3d.io.write_point_cloud(name1, pc1)
    o3d.io.write_point_cloud(name2, pc2)
...
def get_node_occlusion_score(
    ...
    """
    src_points = torch.matmul(src_points + trans.T, rot.T) 
...
def get_node_correspondences(
    ...
    """
    src_nodes = torch.matmul(src_nodes + trans.T, rot.T) 
    src_knn_points = torch.matmul(src_knn_points + (trans.T)[None, ::], rot.T)  
    # generate masks
