...
from roitr_lib.utils import square_distance
...
class FineMatchingLoss(nn.Module):
    ...
    def forward(self, output_dict, data_dict):
        ...
        src_node_corr_knn_points = torch.matmul(src_node_corr_knn_points +  trans.T[None, ::], rot.T)
...
class Evaluator(nn.Module):
    ...
    def evaluate_fine(self, output_dict, data_dict):
        ...
        # print(src_corr_points.shape)
        if src_corr_points.shape[0] == 0:
            precision = 0.
        else:
            src_corr_points = torch.matmul(src_corr_points + trans.T, rot.T)
