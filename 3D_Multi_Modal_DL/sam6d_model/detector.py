...
from sam6d_utils.inout import save_json_bop23
from sam6d_model.utils import BatchedData, Detections, convert_npz_to_json
import time
import glob
from functools import partial
import multiprocessing
import trimesh
from sam6d_model.loss import MaskedPatch_MatrixSimilarity
from sam6d_utils.trimesh_utils import depth_image_to_pointcloud_translate_torch
from sam6d_utils.poses.pose_utils import get_obj_poses_from_template_level
from sam6d_utils.bbox_utils import xyxy_to_xywh, xywh_to_xyxy, compute_iou
from sam6d_utils.visualizer import vis_masks
from roitr_inference import inference
from roitr_model.RIGA_v2 import create_model
from easydict import EasyDict as edict
from configs.utils import load_config
from roitr_lib.utils import depth_to_world, to_ply_file, calc_pc_radius
import math
from roitr_visualizer.plot import update_acc_info_plots
from sam6d_model.utils import lmo_object_ids
...
class Instance_Segmentation_Model(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # Load pretrained RoITr
        config_path = "configs/roitr/inference.yaml"
        cfg = load_config(config_path)
        self.roitr_config = edict(cfg)
        self.roitr = create_model(self.roitr_config).to(self.device) 
        state = torch.load(self.roitr_config.pretrain)
        self.roitr.load_state_dict(state['state_dict'])
        ...
        self.acc_logger = {"acc_dif_sem_score": np.zeros((10,2)), "acc_dif_src_corrs": np.zeros((10,2)), "acc_rel_tgt_corrs": np.zeros((10,2)), "acc_ms": np.zeros((10,2)), "acc_conf": np.zeros((10,2)), "acc_conf_dif": np.zeros((10,2))}
    ...
    def compute_semantic_score(self, proposal_decriptors, detections, batch):
        ...
        # RoITr pass
        ################################################################################################
        cam_k = batch["cam_intrinsic"][0]
        depth_scale = batch["depth_scale"][0].item()
        depth = batch["depth"][0]
        scores_ordered, cads_by_score =  torch.topk(score_per_proposal_and_object, k=score_per_proposal_and_object.shape[1], dim=-1)
        # Iterate through all mask proposals
        for proposal_ind, cads_ranked in enumerate(cads_by_score):
            # Obtain on which rank SAM6D put the ground-truth object
            if self.matches[proposal_ind][1] == -1:
                rank_gt = -1
                continue # This needs to be removed for full evaluation
            else:
                if self.dataset_name == "lmo":
                    tgt_idx = (torch.from_numpy(lmo_object_ids) == self.matches[proposal_ind][1]+1).nonzero(as_tuple=True)[0].item()
                    rank_gt = (cads_ranked == tgt_idx).nonzero(as_tuple=True)[0].item()
                else:
                    rank_gt = (cads_ranked == self.matches[proposal_ind][1]).nonzero(as_tuple=True)[0].item() 

            mask_size = torch.count_nonzero(detections.masks[proposal_ind][0])
            sem_score_top2_diff = scores_ordered[proposal_ind,0] - scores_ordered[proposal_ind,1]
            
            # Pre-Conditions for using RoITr
            if scores_ordered[proposal_ind,0] < self.matching_config.use_roitr_min_thresh or sem_score_top2_diff > self.matching_config.use_roitr_dif_thresh or mask_size < self.roitr_config.min_mask_size:
                if rank_gt == 1:
                    self.failed += 1
                elif rank_gt == 0:
                    self.kept += 1
            ...
            trans = -batch["trans"][0][self.matches[proposal_ind][0]].cpu()
            rot = batch["rot"][0][self.matches[proposal_ind][0]].cpu()
            rot = torch.from_numpy(np.linalg.inv(rot.numpy()))
                
            # Masked Depth Image --> Point Cloud
            ######################################################
            mask = detections.masks[proposal_ind][0]
            bbox = [0, 0, depth.shape[1]-1, depth.shape[0]-1]
            src_pcd = depth_to_world(depth.T.cpu().numpy(), cam_k.cpu().numpy(), depth_scale, bbox, mask.cpu().numpy().T)
            src_pcd = torch.from_numpy(src_pcd)
                        
            # RoITr inference
            ######################################################
            best_perf = best_conf = -1
            corrs = []
            confs = []
            for rank in range(2):
                
                # Get target pointcloud for Roitr
                tgt_pcd = batch["cad_models"][cads_ranked[rank]][0].cpu()

                src_gt = torch.matmul(src_pcd + trans.T, rot.T)
                
                # Inference
                inputs = {
                    'src_points': src_pcd.to(torch.float32),
                    'tgt_points': tgt_pcd.to(torch.float32),
                    'trans':trans,
                    'rot':rot,
                }
                
                conf_scores, src_corr_pts, tgt_corr_pts = inference(self.roitr, inputs, self.device)

                mean_conf = 0
                if len(src_corr_pts) > 0:
                    mean_conf = torch.mean(conf_scores).item()
                conf_dif = abs(mean_conf - best_conf)
                src_corrs = src_corr_pts.unique().shape[0] 

                # get ground-truth correspondences
                # from scipy.spatial import cKDTree
                # norm_rad = calc_pc_radius(tgt_pcd)
                # tree = cKDTree(tgt_pcd)
                # distances, indices = tree.query(src_gt, k=1)
                # close_points = distances < 4.

                # conf_dif = 0
                # rel_tgt_corrs = 0.0
                # best_conf = 0
                # src_corrs = close_points.sum()

                corrs.append(src_corrs)
                confs.append(mean_conf)

                criterion = src_corrs 
                if criterion > best_perf:
                    best_perf = criterion
                    pred_rank = rank

                    rel_tgt_corrs = tgt_corr_pts.unique().shape[0] / tgt_pcd.shape[0]
                if mean_conf > best_conf:
                    best_conf = mean_conf

            # Adapt Semantic scores
            ######################################################
            rel_src_corr_diff = abs(corrs[0] - corrs[1]) / src_pcd.shape[0]
            ...
            if rank_gt >= 0:
                ...
                if pred_rank == rank_gt:
                    self.acc_logger["acc_ms"][min(math.floor(mask_size / 4096.), 9), 0] += 1
                    self.acc_logger["acc_rel_tgt_corrs"][min(math.floor(rel_tgt_corrs / 0.005), 9), 0] += 1
                    self.acc_logger["acc_dif_src_corrs"][min(math.floor(rel_src_corr_diff / 0.004), 9), 0] += 1
                    self.acc_logger["acc_conf_dif"][min(math.floor(conf_dif / 0.004), 9), 0] += 1
                else:
                    self.acc_logger["acc_rel_tgt_corrs"][min(math.floor(rel_tgt_corrs / 0.005), 9), 1] += 1
                    self.acc_logger["acc_dif_src_corrs"][min(math.floor(rel_src_corr_diff / 0.004), 9), 1] += 1
                    self.acc_logger["acc_ms"][min(math.floor(mask_size / 4096.), 9), 1] += 1
                    self.acc_logger["acc_conf_dif"][min(math.floor(conf_dif / 0.004), 9), 1] += 1
                
                if np.argmax(confs) == rank_gt:
                    self.acc_logger["acc_conf"][min(math.floor(best_conf / 0.01), 9), 0] += 1
                else:
                    self.acc_logger["acc_conf"][min(math.floor(best_conf / 0.01), 9), 1] += 1

                if rank_gt == 0:
                    self.acc_logger["acc_dif_sem_score"][min(math.floor(sem_score_top2_diff / 0.01), 9), 0] += 1
                elif rank_gt == 1:
                    self.acc_logger["acc_dif_sem_score"][min(math.floor(sem_score_top2_diff / 0.01), 9), 1] += 1
                
                
                if np.sum(self.acc_logger["acc_dif_src_corrs"]) % 50 == 0:
                    update_acc_info_plots(self.acc_logger, self.dataset_name)                    
                
                # Update scores & Logging variables
                ######################################################
                if pred_rank != 0 \
                and (rel_src_corr_diff >= self.matching_config.roitr_src_corr_diff_thresh \
                and rel_tgt_corrs >= self.matching_config.roitr_tgt_corr_thresh \
                or sem_score_top2_diff <= self.matching_config.roitr_sem_score_thresh):
                    score_per_proposal_and_object[proposal_ind, cads_ranked[0]] = scores_ordered[proposal_ind, pred_rank]
                    score_per_proposal_and_object[proposal_ind, cads_ranked[pred_rank]] = scores_ordered[proposal_ind, 0]
                    if rank_gt == pred_rank:
                        self.corrected += 1
                    elif rank_gt == 0:
                        self.destroyed += 1
                else:
        ...
        print("Corrected", self.corrected, "Kept", self.kept, "Destroyed", self.destroyed, "Hopeless", self.hopeless, "Failed", self.failed, "RoITr bonus:", self.corrected-self.destroyed)
    ...
    def test_step(self, batch, idx):
        ...
        gt_bboxes = torch.tensor(batch["gt_bboxes"]).cpu()
        gt_masks = batch["gt_masks"][0]
        gt_obj_ids = batch["gt_obj_ids"][0].cpu()
        ...
        # Filter masks and define ground-truth object
        self.matches = []
        for i, mask in enumerate(detections.masks):
            ...
            for j, gt_mask in enumerate(gt_masks):
                iou = torch.sum(torch.logical_and(mask, gt_mask)) / torch.sum(torch.logical_or(mask, gt_mask))
                if iou >= largest_iou:
                    largest_iou = iou
                    best_ind = j
                    best_mask = gt_mask

            mask_size = torch.count_nonzero(best_mask)
            if largest_iou >= 0.5 and mask_size >= 4096 and batch["visib_fract"][best_ind] >= 0.5:
                self.matches.append((best_ind, gt_obj_ids[best_ind] - 1))
            else:
                self.matches.append((best_ind,-1)) # these samples are not included in the plots
        ...
        (
            ...
            semantic_score,
            best_template,
        ) = self.compute_semantic_score(query_decriptors, detections, batch)
