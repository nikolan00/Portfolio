...
from sam6d_provider.base_bop import BaseBOP
import cv2
import imageio as imageio
from sam6d_utils.inout import load_json
...
class BaseBOPTest(BaseBOP):
    ...
    def load_depth_img(self, idx):
        ...
        depth = cv2.imread(os.path.join(depth_path), cv2.IMREAD_ANYDEPTH)
        # depth = np.clip(depth.flatten(), np.median(depth), np.median(depth) + 1000)
        # print(depth[(depth >= np.median(depth)) & (depth <= np.median(depth) + 500)])
        # exit(0)
    ...
    def __getitem__(self, idx):
        ...
        gt_bboxes = self.metaData.iloc[idx]["gt_bboxes"]
        visib_fract = self.metaData.iloc[idx]["visib_fract"]
        gt_trans = np.array(self.metaData.iloc[idx]["gt_trans"]).reshape(-1,3,1)
        gt_rot = np.array(self.metaData.iloc[idx]["gt_rot"]).reshape(-1,3,3)
        gt_obj_ids = np.array(self.metaData.iloc[idx]["gt_obj_ids"])
        ...
        for i in range(len(gt_bboxes)):
            mask_id =  str(frame_id).zfill(6) + "_" + str(i).zfill(6) + ".png"
            mask_path = os.path.join("/".join(rgb_path.split("/")[:-2]), "mask_visib", mask_id)
            mask_image = cv2.imread(mask_path, cv2.IMREAD_ANYDEPTH)
