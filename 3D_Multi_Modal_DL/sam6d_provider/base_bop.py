...
import os.path as osp
import pandas as pd
from sam6d_utils.inout import load_json, save_json, casting_format_to_save_json
from sam6d_utils.poses.pose_utils import load_index_level_in_level2
import torch
from sam6d_utils.bbox_utils import CropResizePad
...
OBJ_IDS = {
    ...
    "poseverse": list(range(18649, 19162))
}
...
class BaseBOP(Dataset):
    ...
    def load_metaData(self, reset_metaData, mode="query", split="test", level=2):
            ...
            if reset_metaData:
                for scene_path in tqdm(self.list_scenes, desc="Loading metaData"):
                    ...
                    for idx_frame in range(len(rgb_paths)):
                        ...
                        gt_bboxes = [
                            x["bbox_visib"] for x in scene_gt_info[f"{id_frame}"]
                        ]
                        gt_obj_ids = [
                            x["obj_id"] for x in scene_gt[f"{id_frame}"]
                        ]
                        gt_rot = [
                            x["cam_R_m2c"] for x in scene_gt[f"{id_frame}"]
                        ]
                        gt_trans = [
                            x["cam_t_m2c"] for x in scene_gt[f"{id_frame}"]
                        ]
                        visib_fract = [
                            x["visib_fract"] for x in scene_gt_info[f"{id_frame}"]
                        ]
                        metaData["scene_id"].append(scene_id)
                        metaData["gt_bboxes"].append(gt_bboxes)
                        metaData["gt_trans"].append(gt_trans)
                        metaData["gt_rot"].append(gt_rot)
                        metaData["visib_fract"].append(visib_fract)
                        metaData["gt_obj_ids"].append(gt_obj_ids)
        ...
        for model_name in os.listdir(MODELS_PATH):
            if model_name.endswith(".ply"):
                mesh = o3d.io.read_triangle_mesh(os.path.join(MODELS_PATH, model_name))     
                pcd = mesh.sample_points_uniformly(number_of_points=8192)           
                self.cad_models.append(np.asarray(pcd.points))
        
        self.cad_normals = []
        for pcd in self.cad_models:
            o3d_pcd = to_o3d_pcd(pcd)
            o3d_pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=33))
            normals = normal_redirect(pcd, np.asarray(o3d_pcd.normals), view_point=np.array([0., 0., 0.]))
            self.cad_normals.append(normals)
