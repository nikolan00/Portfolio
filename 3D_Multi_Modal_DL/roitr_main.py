...
from roitr_lib.utils import setup_seed
from configs.utils import load_config
from easydict import EasyDict as edict
from roitr_dataset.dataloader import get_dataset, get_dataloader
from roitr_model.RIGA_v2 import create_model
from roitr_lib.loss import OverallLoss, Evaluator
from roitr_lib.tester import get_trainer
...
def main():
    ...
    config['snapshot_dir'] = 'roitr_snapshot/{}'.format(config['exp_dir'])
    config['tboard_dir'] = 'roitr_snapshot/{}/tensorboard'.format(config['exp_dir'])
    config['save_dir'] = 'roitr_snapshot/{}/checkpoints'.format(config['exp_dir'])
    config['visual_dir'] = 'roitr_snapshot/{}/visualization'.format(config['exp_dir'])
    ##########################################################
    config = edict(config)
    ...
    if config.local_rank <= 0:
        ...
        os.system(f'cp -r roitr_model {config.snapshot_dir}')
        os.system(f'cp -r roitr_dataset {config.snapshot_dir}')
        os.system(f'cp -r roitr_lib {config.snapshot_dir}')
        shutil.copy2('roitr_main.py', config.snapshot_dir)
    ##################################################################
    # create model
