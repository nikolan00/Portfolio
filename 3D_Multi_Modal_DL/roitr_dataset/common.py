...
def collate_fn(list_data, config):
    ...
    for ind, (src_pcd, tgt_pcd, src_normals, tgt_normals, src_feats, tgt_feats, rot, trans, raw_src_pcd, metric_index, obj_id, og_id, mask, bbox) in enumerate(list_data):
        ...
        batched_obj_ids.append(obj_id)
        batched_og_ids.append(og_id)
    ...
    batched_masks = torch.from_numpy(np.concatenate(batched_masks, axis=0))
    batched_bboxes = torch.from_numpy(np.concatenate(batched_bboxes, axis=0))
    ...
    batched_obj_ids = torch.tensor(batched_obj_ids)
    batched_og_ids = torch.tensor(batched_og_ids)
    ...
    inputs = {
        ...
        'obj_id': batched_obj_ids,
        'og_id': batched_og_ids,
        'mask': batched_masks,
        'bbox' : batched_bboxes,
    }
    return inputs 
