...
from roitr_cpp_wrappers.pointops.functions import pointops
from roitr_lib.utils import calc_ppf_gpu, group_all, point_to_node_partition, get_node_correspondences, index_select, get_node_occlusion_score, to_o3d_pcd, pps_sampling
import torch.nn.functional as F 
from roitr_model.transformer.ppftransformer import LocalPPFTransformer, PPFTransformer
from roitr_model.transformer.geotransformer import GeometricTransformer
...
class TransitionDown(nn.Module):
    ...
    def forward(self, pxon):
        p, x, o, n, _, _, fps = pxon  # (n, 3), (n, c), (b), (n, 3)
        if self.stride != 1:
            ...
            if fps:
                ...
                # idx = pointops.furthestsampling(p, o, n_o).long()  # (m)
                idx = pps_sampling(p, p.shape[0]//self.stride)
                # np.random.seed(42)
                # idx = torch.tensor(np.random.permutation(p.shape[0])[:p.shape[0]//self.stride])
...
class RIPointTransformer(nn.Module):
    ...
    def forward(self, s_pxon, t_pxon, src_deformed_pcd):
        ...
        ############################
        # source pcd
        s_p1, s_x1, s_o1, s_n1, s_idx1, s_ppf1, _ = self.enc1([s_p0, s_x0, s_o0, s_n0, None, None, False])
        s_p2, s_x2, s_o2, s_n2, s_idx2, s_ppf2, s_d_idx2 = self.enc2([s_p1, s_x1, s_o1, s_n1, None, None, False])
        s_p3, s_x3, s_o3, s_n3, s_idx3, s_ppf3, s_d_idx3 = self.enc3([s_p2, s_x2, s_o2, s_n2, None, None, False])
        s_p4, s_x4, s_o4, s_n4, s_idx4, s_ppf4, s_d_idx4 = self.enc4([s_p3, s_x3, s_o3, s_n3, None, None, False])

        # target pcd
        t_p1, t_x1, t_o1, t_n1, t_idx1, t_ppf1, _ = self.enc1([t_p0, t_x0, t_o0, t_n0, None, None, True])
        t_p2, t_x2, t_o2, t_n2, t_idx2, t_ppf2, _ = self.enc2([t_p1, t_x1, t_o1, t_n1, None, None, True])
        t_p3, t_x3, t_o3, t_n3, t_idx3, t_ppf3, _ = self.enc3([t_p2, t_x2, t_o2, t_n2, None, None, True])
        t_p4, t_x4, t_o4, t_n4, t_idx4, t_ppf4, _ = self.enc4([t_p3, t_x3, t_o3, t_n3, None, None, True])
        ###########################
