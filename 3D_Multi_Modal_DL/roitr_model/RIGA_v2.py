...
from roitr_model.model import *
from roitr_model.modules import CoarseMatching, AdaptiveSuperPointMatching, GTCoarseCorrGenerator, FineMatching, LearnableLogOptimalTransport
import torch.nn.functional as F
from roitr_lib.utils import to_o3d_pcd, to_ply_file
...
class RIGA_v2(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.benchmark not in ['4DMatch', '4DLoMatch']:
    ...
    def forward(self, src_pcd, tgt_pcd, src_feats, tgt_feats, src_normals, tgt_normals, rot, trans, src_raw_pcd, inference=False):
        ...
        radius = self.config.matching_radius
        output_dict = {}
        # 1. get descriptors
        ...
        if not inference:
            ...
            # Necessary for testing with distractor object
            while gt_node_corr_overlaps.shape[0] < 1:
                radius *= 2
                # print(f"{gt_node_corr_overlaps.shape[0]} corrs, attempting with {radius}")
                # src_gt = torch.matmul(src_pcd + trans.T, rot.T) 
                # to_ply_file(src_gt, tgt_pcd)
                # exit(0)
                gt_node_corr_indices, gt_node_corr_overlaps = get_node_correspondences(
                    tgt_node_xyz,
                    src_node_xyz,
